...
float dx(int i, float* t, float Tens, float Bias, float Cont, Figura* Fig)
{
	...
	if (i % 2 == 0)
		return  0.5 * (1 - Tens) * (1 + Bias) * (1 + Cont) * (Fig->vertex.at(i).x - Fig->vertex.at(i - 1).x) / (t[i] - t[i - 1]) + 0.5 * (1 - Tens) * (1 - Bias) * (1 - Cont) * (Fig->vertex.at(i + 1).x - Fig->vertex.at(i).x) / (t[i + 1] - t[i]);
	else
		return  0.5 * (1 - Tens) * (1 + Bias) * (1 - Cont) * (Fig->vertex.at(i).x - Fig->vertex.at(i - 1).x) / (t[i] - t[i - 1]) + 0.5 * (1 - Tens) * (1 - Bias) * (1 + Cont) * (Fig->vertex.at(i + 1).x - Fig->vertex.at(i).x) / (t[i + 1] - t[i]);
}
